...
tinify.key = "dF3zPRV0BQdQR8mQ6Qm8xdJy0jSVNRQY"		# API KEY
version = "1.1.0"		# 版本号
author = "whz"          # 修改者
voice = True			# 是否语音提示
...
#	print "file = %s" %inputFile
	...
	if fileSuffix == '.png' or fileSuffix == '.jpg' or fileSuffix == '.jpeg':
		compress_core(inputFile,"tiny_"+basename, width)
	else:
		say("不支持该文件类型!")
...
@click.option('-e', "--email", type=str, prompt="随便填", help="可以找到我奥")
def run(file, dir, width,email):
	print("GcsSloop TinyPng V '"+version+"',修改者: '"+author+"'")
	say("启动压缩工具!")
	if file is not None:
		say("压缩指定文件!")
		...
		say("如需压缩图片，请联系727906773@qq.com")
		pass
	else:
		say("压缩当前目录下所有图片文件")
